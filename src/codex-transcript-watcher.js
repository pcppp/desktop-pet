...
function isCodexReplyFinishedRecord(record) {
...
function createCodexTranscriptWatcher(options = {}) {
  const rootDir = options.rootDir || path.join(os.homedir(), ".codex", "sessions");
  ...
  const onError = typeof options.onError === "function"
    ? options.onError
    : (error) => {
      console.error("Codex transcript watcher error:", error);
    };
